...
`Fetch Groups` and `Sync Playlists` download each configured provider playlist into the `/work` volume and rescan the groups. The latest group summary is saved to `/work/web-groups.json` and restored when the page reloads. Generator runs reuse the cached playlist files; they do not refresh playlists automatically unless no cached file exists yet.
